Siin on sinu GitHub README täpselt üks-ühele inglisekeeles:

markdown# Hi there! 👋 I'm Kevin Rattasepp

## 🎯 Starting My Data Analyst Career

I am currently studying in the DACA (Data Analyst Career Accelerator) program, where I focus on data cleaning, modeling, and finding business value from data.

## 🛠️ Technologies I'm Learning

![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)

- **Databases:** PostgreSQL, Supabase
- **Programming Languages:** Python (pandas, plotly, numpy)
- **Visualization:** Power BI, Streamlit, MatPlotLib
- **Version Control:** Git, GitHub
- **IDE:** Visual Studio Code (VS Code)
- **Notebooks:** Jupyter Notebooks (.ipynb)

## 📂 Projects

- [**DACA Portfolio**](https://github.com/rattaseppkevin-tech/daca-portfolio) - 11-week learning projects
- [**DACA Team Portfolio**](https://github.com/rattaseppkevin-tech/urbanstyle-TOOTE-grupp) - 11-week learning projects
- [**Udemy Data Analysis**](https://github.com/rattaseppkevin-tech/Data-Analysis) - Ongoing learning.
- [**TalTech Python Basics**](https://github.com/rattaseppkevin-tech/python-basics-taltech) - 26 academic hours

## 📈 Goal
To apply Python and SQL capabilities to transform raw data into clear business decisions.

## 📫 Contact

- **Email:** rattaseppkevin@gmail.com
- **LinkedIn:** [linkedin.com/in/kevin-rattasepp](https://www.linkedin.com/in/kevin-rattasepp-5261733ab/)

---

_"Data is the new oil, but analytics is the combustion engine." – Peter Sondergaard_
